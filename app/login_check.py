...
def check_need_login():
    ...
    if request.endpoint.split('.')[0] in ('wechat', 'wechatcoursewares', 'api', 'error_404', 'test', 'about'):
        return
    #教师限制访问
    if session.get('role', '') == 'Teacher' and session.get('admin', '') !=  9:
        allowlist = ('teacher', 'teachercenter', 'editcourses', 'editcoursewares', 'edithomework', 'editexperiment', 'experiment',\
    ...
    if session.get('role', '') == 'Student':
        allowlist = ('student', 'studentcenter', 'experiment',\
        'courses', 'coursewares', 'markdowneditor', 'user_base', 'about', 'help', 'index', 'homework')
        if request.endpoint.split('.')[0] not in allowlist:
