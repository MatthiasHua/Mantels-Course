...
from app import db
#创建应用实例
wechatcoursewares = Blueprint('wechatcoursewares', __name__,  template_folder='templates')


@wechatcoursewares.route('/id/<int:id>/chapter/<int:chapter>', methods=['POST', 'GET'])
def editbychapter(id, chapter):
    ...
    return render_template("Wechat_Courseware_Chapter.html",\
    chapters = chapters,\
    lessons = lessons,\
...
@wechatcoursewares.route('/id/<int:id>/chapter/<int:chapter>/lesson/<int:lesson>', methods=['POST', 'GET'])
def editbylesson(id, chapter, lesson):
    ...
    return render_template("Wechat_Courseware_Lesson.html",\
    chapters = chapters,\
    lessons = lessons,\
    role = session.get('role', 'unknow'),\
    username = session.get('username', ''),\
